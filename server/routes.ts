...
// Dashboard Items Routes - CRUD Operations
export const dashboardItemsRoutes = {
  async getItems(userId: string) {
    try {
      const items = await db
        .select()
        .from(dashboardItems)
        .where(eq(dashboardItems.userId, userId as any))
        .orderBy(asc(dashboardItems.createdAt));
      return items;
    } catch (error) {
      throw new Error(`Failed to fetch dashboard items: ${error}`);
    }
  },
  ...
  async getItem(itemId: string) {
    try {
      const item = await db
        .select()
        .from(dashboardItems)
        .where(eq(dashboardItems.id, itemId as any))
        .limit(1);
      return item[0] || null;
    } catch (error) {
      throw new Error(`Failed to fetch dashboard item: ${error}`);
    }
  },
  ...
  async createItem(data: any) {
    try {
      ...
      const created = await db.insert(dashboardItems).values(item).returning();
      return created[0] || null;
    } catch (error) {
      throw new Error(`Failed to create dashboard item: ${error}`);
    }
  },
  ...
  async updateItem(itemId: string, data: any) {
    try {
      ...
      throw new Error(`Failed to update dashboard item: ${error}`);
    }
  },

  async deleteItem(itemId: string) {
    try {
      await db.delete(dashboardItems).where(eq(dashboardItems.id, itemId as any));
      return { success: true };
    } catch (error) {
      throw new Error(`Failed to delete dashboard item: ${error}`);
    }
  },
