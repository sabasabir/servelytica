...
export function setupApiRoutes(app: any) {
  ...
  app.post('/api/videos/upload-url', async (req: any, res: any) => {
    try {
      const { videoLink, userId, title, description, focusArea, coachIds, platform, thumbnailUrl, embedUrl } = req.body;
      
      if (!videoLink || !userId) {
        return sendError(res, 'Missing videoLink or userId', 400);
      }
      ...
      try {
        ...
        if (!['http:', 'https:'].includes(url.protocol)) {
          return sendError(res, 'Invalid video URL protocol', 400);
        }
        ...
        const isKnownPlatform = allowedDomains.some(domain => url.hostname.includes(domain));
        const isDirectVideo = /\.(mp4|webm|mov|avi|mkv)$/i.test(url.pathname);
        
        if (!isKnownPlatform && !isDirectVideo) {
          console.log(`[URL_UPLOAD] Unrecognized platform: ${url.hostname}, allowing anyway`);
        }
      } catch {
        return sendError(res, 'Invalid video URL format', 400);
      }
      
      console.log(`[URL_UPLOAD] Processing video URL upload: ${videoLink}`);
      ...
      console.log(`[URL_UPLOAD] Video data to insert:`, videoData);
      
      let createdVideo;
      try {
        createdVideo = await videoRoutes.createVideo(videoData);
      } catch (dbError) {
        console.error(`[URL_UPLOAD] Database error creating video:`, dbError);
        throw new Error(`Failed to create video record: ${dbError instanceof Error ? dbError.message : String(dbError)}`);
      }
      ...
      console.log(`[URL_UPLOAD] Video record created with ID: ${createdVideo.id}`);
      ...
      if (coachIds && Array.isArray(coachIds) && coachIds.length > 0) {
        try {
          ...
          console.log(`[URL_UPLOAD] Assigning coaches:`, coachAssignments);
          await db.insert(videoCoaches).values(coachAssignments);
          
          await videoRoutes.updateVideo(createdVideo.id, { analyzed: true });
          createdVideo.analyzed = true;
          console.log(`[URL_UPLOAD] Coaches assigned successfully`);
        } catch (coachError) {
          console.warn('[URL_UPLOAD] Warning: Failed to assign coaches, but video was created:', coachError);
        }
      }
      ...
      console.log(`[URL_UPLOAD] Sending response:`, response);
      sendJson(res, response, 201);
    } catch (error) {
      console.error('[URL_UPLOAD] Error uploading video URL:', error);
      console.error('[URL_UPLOAD] Stack trace:', error instanceof Error ? error.stack : 'N/A');
      sendError(res, error, 500);
    }
  });
