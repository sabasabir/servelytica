...
-- DISABLES RLS ON EVERY TABLE IN THE DATABASE
-- ============================================
...
ALTER TABLE IF EXISTS public.articles_with_counts DISABLE ROW LEVEL SECURITY;
...
-- Drop ALL RLS policies from every table
DO $$ 
DECLARE 
  r RECORD;
BEGIN
  FOR r IN (SELECT schemaname, tablename FROM pg_tables WHERE schemaname = 'public') LOOP
    EXECUTE 'DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON ' || r.schemaname || '.' || r.tablename;
    EXECUTE 'DROP POLICY IF EXISTS "Enable read access for all users" ON ' || r.schemaname || '.' || r.tablename;
    EXECUTE 'DROP POLICY IF EXISTS "Enable insert for authenticated users" ON ' || r.schemaname || '.' || r.tablename;
    EXECUTE 'DROP POLICY IF EXISTS "Enable insert for authenticated" ON ' || r.schemaname || '.' || r.tablename;
    EXECUTE 'DROP POLICY IF EXISTS "Enable read for all users" ON ' || r.schemaname || '.' || r.tablename;
  END LOOP;
END $$;
